...
class InitrealtimeScreen extends Component {
  constructor(props) {
    super(props);
    this.state = {
          ...
          <View>
            <View style={styles.block}>
              <Text style={{ fontSize: typography.MD_TEXT, color: colors.TEXT, textAlign: "left", fontFamily: typography.FONT_DEFALT, fontWeight: "bold" }}>
                {"\n"}Great! Now I know how to want to practice speaking.{"\n"}{"\n"}(B.){"\n"}
              </Text>
              <Text style={{ fontSize: typography.MD_TEXT, color: colors.TEXT, textAlign: "left", fontFamily: typography.FONT_DEFALT }}>
                You can change the speed I am limiting you to with the slider here.{"\n"}{"\n"}Target Speed:{"\n"}{"\n"}Between 120 to 200 SPM (syllables per minute) is average fluent speech.{"\n"}{"\n"}This is an example of prolonged speech method:
              </Text>
              <View style={{ flex: 1, alignItems: 'stretch', justifyContent: 'center' }}>
                <Text style={{ fontSize: typography.MD_TEXT, color: colors.TEXT, textAlign: "left", fontFamily: typography.FONT_DEFALT, fontWeight: "bold" }}>{this.state.current_speed}</Text>
                <Slider
                  minimumTrackTintColor={'#909090'}
                  maximumTrackTintColor={'#909090'}
                  step={20}
                  thumbTintColor={colors.BUTTON_LOGIN}
                  minimumValue={40}
                  maximumValue={240}
                  value={this.state.current_speed}
                  onValueChange={(value) => this.setState({ current_speed: value })}
                />
              </View>
              ...
              />
              <Text style={{ fontSize: typography.MD_TEXT, color: colors.TEXT, textAlign: "left", fontFamily: typography.FONT_DEFALT, fontWeight: "bold" }}>
                {"\n"}Press the Next button after picking your speed to practice at.</Text>
            </View>
            <View style={styles.displayInlineBlock}>
              <Button
                ...
                buttonWidth={'30%'}
                background={colors.BUTTON_LOGIN}
                onPress={() => NavigationService.navigate("Init2realtime", {
...
export default InitrealtimeScreen
